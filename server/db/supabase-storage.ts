...
export class SupabaseStorage implements Storage {
  ...
  async getMoneyTransfers(companyId?: number): Promise<MoneyTransfer[]> {
    ...
    if (!schema.moneyTransfersTable) {
      throw new Error('Money transfers table not available in current schema');
    }
    
    let query = db.select().from(schema.moneyTransfersTable);
    
    if (companyId) {
      query = query.where(eq(schema.moneyTransfersTable.companyId, companyId));
    }
    
    const transfers = await query.orderBy(desc(schema.moneyTransfersTable.transferDate));
    return transfers;
  }
  ...
  async createMoneyTransfer(transfer: NewMoneyTransfer): Promise<MoneyTransfer> {
    ...
    const result = await db.insert(schema.moneyTransfersTable)
      .values(transfer)
      .returning();
  ...
  async updateMoneyTransfer(id: number, transfer: Partial<MoneyTransfer>): Promise<MoneyTransfer | null> {
    ...
    const result = await db.update(schema.moneyTransfersTable)
      .set(transfer)
      .where(eq(schema.moneyTransfersTable.id, id))
      .returning();
  ...
  async deleteMoneyTransfer(id: number): Promise<boolean> {
    ...
    await db.delete(schema.moneyTransfersTable)
      .where(eq(schema.moneyTransfersTable.id, id));
