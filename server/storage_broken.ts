...
export class SupabaseMultiStorage implements IStorage {
  ...
  async getAppointments(userId: number): Promise<Appointment[]> {
    // Retornar agendamentos baseados na categoria do usuário
    // Para demonstração, vamos usar todos os agendamentos e filtrar no frontend
    return this.getAllMockAppointments();
  }

  private getAllMockAppointments(): Appointment[] {
    // Retorna todos os agendamentos mockados que serão filtrados no frontend por categoria
    return [
      // Pet & Veterinário
      {
        id: 1,
