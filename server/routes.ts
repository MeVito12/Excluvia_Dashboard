...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // === ROTAS DE HIERARQUIA EMPRESARIAL ===

  // Empresas
  app.get("/api/hierarchy/companies", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      const { createdBy } = req.query;
      const companies = await storage.getCompanies(createdBy ? Number(createdBy) : undefined);
      res.json(companies);
    } catch (error) {
  ...
  app.get("/api/hierarchy/companies/:id", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      const company = await storage.getCompanyById(Number(req.params.id));
      if (!company) {
  ...
  app.post("/api/hierarchy/companies", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
  ...
  // Usuários com hierarquia - usando dados reais do banco
  app.get("/api/hierarchy/users", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      
      // Buscar todos os usuários com suas hierarquias via SQL
      const result = await (storage as any).db?.select(`
        SELECT 
          u.id, u.name, u.email, u.user_type, u.company_id, u.branch_id, u.manager_id, u.is_active,
          c.name as company_name, c.business_category,
      ...
      if (result) {
        ...
        // Fallback para dados mockados se não conseguir acessar o banco
        const mockUsers = [
          {
            id: 1,
  ...
  app.get("/api/hierarchy/users/:id", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      const user = await storage.getUserWithHierarchy(Number(req.params.id));
      if (!user) {
  ...
  app.get("/api/hierarchy/companies/:companyId/users", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      const users = await storage.getUsersByCompany(Number(req.params.companyId));
      res.json(users);
    } catch (error) {
  ...
  app.get("/api/hierarchy/users/:userId/subordinates", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      const subordinates = await storage.getUserSubordinates(Number(req.params.userId));
  ...
  // Filiais por empresa
  app.get("/api/hierarchy/companies/:companyId/branches", async (req, res) => {
    try {
      const storage = await databaseManager.getStorage();
      const branches = await storage.getBranchesByCompany(Number(req.params.companyId));
      res.json(branches);
    } catch (error) {
